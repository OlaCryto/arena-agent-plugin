...
class SignalsModule {
    ...
    async getAssetContexts() {
        if (this.assetCache && Date.now() - this.assetCache.ts < 10000) {
     ...
     */
    async getFundingHistory(coin, hours = 24) {
     ...
     */
    async getWhalePositions(coin, minPositionUsd = 100000) {
     ...
     */
    async getWhaleTrades(coin, minSizeUsd = 50000, count = 20) {
    ...
    async getSignalSummary(coin) {
        ...
        let netBias = "neutral";
        if (longValue > shortValue * 1.3)
            netBias = "long";
        else if (shortValue > longValue * 1.3)
            netBias = "short";
        // Generate verdict
        const reasons = [];
        let longScore = 0;
        ...
        if (technical.rsi14 < 30) {
            longScore += 2;
            reasons.push(`RSI ${technical.rsi14} — oversold, potential bounce`);
        }
        else if (technical.rsi14 > 70) {
            shortScore += 2;
            reasons.push(`RSI ${technical.rsi14} — overbought, potential pullback`);
        }
        else if (technical.rsi14 < 40) {
            longScore += 1;
            reasons.push(`RSI ${technical.rsi14} — approaching oversold`);
        }
        else if (technical.rsi14 > 60) {
            shortScore += 1;
            reasons.push(`RSI ${technical.rsi14} — approaching overbought`);
        }
        ...
        if (netBias === "long") {
            longScore += 1;
            reasons.push("Whale orderbook bias: more large bids than asks");
        }
        else if (netBias === "short") {
            shortScore += 1;
            reasons.push("Whale orderbook bias: more large asks than bids");
        }
        // Book depth
        if (whaleTrades[0]) {
            ...
            if (ratio > 1.5) {
                longScore += 1;
                reasons.push(`Bid/ask depth ratio ${ratio} — strong buy support`);
            }
            else if (ratio < 0.67) {
                shortScore += 1;
                reasons.push(`Bid/ask depth ratio ${ratio} — heavy sell pressure`);
            }
        }
        // 24h momentum
        if (market.change24hPct > 3) {
            longScore += 1;
            reasons.push(`+${market.change24hPct}% in 24h — strong momentum`);
        }
        else if (market.change24hPct < -3) {
            shortScore += 1;
            reasons.push(`${market.change24hPct}% in 24h — selling momentum`);
        }
        let direction = "wait";
        let confidence = "low";
     ...
     */
    async scanOpportunities(count = 5) {
        ...
        const summaries = await Promise.all(Array.from(candidates).slice(0, 15).map((coin) => this.getSignalSummary(coin).catch(() => null)));
