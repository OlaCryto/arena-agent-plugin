...
export function resolveToken(input: string): TokenInfo | null {
  ...
  if (POPULAR_TOKENS[upper]) return POPULAR_TOKENS[upper];

  // Check case-sensitive keys (for USDC.e, BTC.b, etc.)
  for (const [key, info] of Object.entries(POPULAR_TOKENS)) {
    if (key.toLowerCase() === input.toLowerCase()) return info;
  }

  // Check by address
  for (const info of Object.values(POPULAR_TOKENS)) {
    if (info.address.toLowerCase() === input.toLowerCase()) return info;
  }
